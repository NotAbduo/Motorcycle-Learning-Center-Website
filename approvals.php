...
/* ───── Handle checkbox-based batch approval/denial ───── */
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    foreach ($_POST['logs'] as $logId => $actions) {
        ...
        // Skip if both or neither are checked
        if ($approve && $deny) {
        ...
        if ($approve) {
            try {
                ...
                $copyStmt = $pdo->prepare("
                    INSERT INTO approval_logs
                          (trainee_id, instructor_id, training_hours, start_time, end_time, request_date)
                    SELECT trainee_id, instructor_id, training_hours, start_time, end_time, request_date
                      FROM waiting_logs
                     WHERE log_id = :id
                    LIMIT 1
                ");
                ...
                $delStmt = $pdo->prepare("DELETE FROM waiting_logs WHERE log_id = :id");
            ...
            try {
                $stmt = $pdo->prepare("DELETE FROM waiting_logs WHERE log_id = :id");
...
$stmt = $pdo->query("
    SELECT wl.log_id,
           ...
           wl.start_time,
           wl.end_time,
           DATE_FORMAT(wl.request_date, '%Y-%m-%d') AS req_date,
           t.name AS trainee_name,
           e.name AS instructor_name
      FROM waiting_logs wl
 LEFT JOIN trainees t ON wl.trainee_id = t.national_id
 LEFT JOIN employees e ON wl.instructor_id = e.national_id
  ORDER BY wl.request_date DESC, wl.log_id DESC
");
...
<head>
<meta charset="utf-8">
<title>Pending Training-Hour Requests</title>
<link rel="stylesheet" href="css/hours.css">
<link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.0/css/all.min.css">
<meta name="viewport" content="width=device-width, initial-scale=1.0">
<style>
    /* Make checkboxes slightly bigger for better UX */
    input[type="checkbox"] {
        transform: scale(1.2);
        cursor: pointer;
    }
...
<div class="page-content">
  <div class="dashboard-container">
    <h1 class="text-xl font-bold mb-4">Waiting Logs</h1>
    ...
    <table class="table-auto w-full border mb-6">
        ...
        <?php foreach ($logs as $row): ?>
            <tr class="border-t text-center">
                ...
                <td>
                    <input type="checkbox" class="approve-checkbox" name="logs[<?= $row['log_id'] ?>][approve]">
                </td>
                <td>
                    <input type="checkbox" class="deny-checkbox" name="logs[<?= $row['log_id'] ?>][deny]">
                </td>
            </tr>
        <?php endforeach; ?>
        </tbody>
