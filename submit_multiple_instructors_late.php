...
// Fetch active instructors from the employees table
$stmt = $conn->prepare("SELECT name, national_id FROM employees WHERE is_active = 1 ORDER BY name ASC");
$stmt->execute();
...
<head>
    <meta charset="UTF-8">
    <title>Select Instructors</title>
...
<div class="dashboard-container">
    ...
    <form id="instructorForm" method="POST" action="select_company_late.php">
        <input type="hidden" name="selected_instructors" id="selectedInstructors">
            ...
            <table>
                ...
                <tbody>
                    <?php foreach ($instructors as $inst): ?>
                        <tr>
                            <td>
                                <input type="checkbox" class="instructor-checkbox"
                                       data-name="<?= htmlspecialchars($inst['name']) ?>"
                                       data-id="<?= htmlspecialchars($inst['national_id']) ?>">
                            </td>
                            <td><?= htmlspecialchars($inst['name']) ?></td>
                            <td><?= htmlspecialchars($inst['national_id']) ?></td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
...
<script>
    ...
    function submitSelected() {
        ...
        document.getElementById('selectedInstructors').value = JSON.stringify(selected);
